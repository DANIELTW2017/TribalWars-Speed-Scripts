// ==UserScript==
// @name         NovoFarmJogadores
// @include      https://*&screen=overview_villages&mode=combined*
// ==/UserScript==
...
setTimeout(function () {
  ...
  function resErro () {
    let alvosAtaque = Lockr.get("Alvos");
    Lockr.set("Alvos_Nao_Atacados", alvosAtaque);
  }

  function selecionarAlvos () {
    let alvos = Lockr.get("Alvos_Nao_Atacados");
        ...
        for (let minha of minhas) {
            ...
            let auxOk = false;
                ...
                if (alvos.length < 1) {
                resErro();
                alvos = Lockr.get("Alvos_Nao_Atacados");
                }
            ...
            Lockr.set("Alvos_Nao_Atacados", alvos);
            primeira();
...
/* Script Escrito por ThiioM :) */
                $.ajax({
                    ...
                    error: function () {
                        window.location.assign("https://brs1.tribalwars.com.br/game.php?village={game}&screen=overview_villages&mode=combined&group="+grupo+"&page=0&&"); 
                    }
                });
  ...
  function segunda() {
    ...
    let i = 0;
        ...
        for (let minha of minhas) {
            ...
            _Data.spear = parseInt(Lockr.get('Tropas_Ataque_Lanceiro')) < parseInt(minha.lanceiros) ? Lockr.get('Tropas_Ataque_Lanceiro') : minha.lanceiros;
            _Data.sword = parseInt(Lockr.get('Tropas_Ataque_Espadachim')) < parseInt(minha.espadachins) ? Lockr.get('Tropas_Ataque_Espadachim') : minha.espadachins;
            _Data.axe = parseInt(Lockr.get('Tropas_Ataque_Barbaro')) < parseInt(minha.barbaros) ? Lockr.get('Tropas_Ataque_Barbaro') : minha.barbaros;
            if (temArqueiro) {
                _Data.archer = parseInt(Lockr.get('Tropas_Ataque_Arqueiro')) < parseInt(minha.arqueiro) ? Lockr.get('Tropas_Ataque_Arqueiro') : minha.arqueiro;
            }
            _Data.spy = parseInt(Lockr.get('Tropas_Ataque_Explorador')) < parseInt(minha.exploradores) ? Lockr.get('Tropas_Ataque_Explorador') : minha.exploradores;
            _Data.light =  parseInt(Lockr.get('Tropas_Ataque_Cavalaria_leve:')) < parseInt(minha.cavLeves) ? Lockr.get('Tropas_Ataque_Cavalaria_leve:') : minha.cavLeves;
            if (temArqueiro) {
                _Data.marcher = parseInt(Lockr.get('Tropas_Ataque_Arqueiro_a_cavalo')) < parseInt(minha.cavArqueiro) ? Lockr.get('Tropas_Ataque_Arqueiro_a_cavalo') : minha.cavArqueiro;
            }
            _Data.heavy = parseInt(Lockr.get('Tropas_Ataque_Cavalaria_pesada')) < parseInt(minha.cavPesadas) ? Lockr.get('Tropas_Ataque_Cavalaria_pesada') : minha.cavPesadas;
            _Data.ram = parseInt(Lockr.get('Tropas_Ataque_Ariete')) < parseInt(minha.arietes) ? Lockr.get('Tropas_Ataque_Ariete') : minha.arietes;
            _Data.catapult = parseInt(Lockr.get('Tropas_Ataque_Catapulta')) < parseInt(minha.catapultas) ? Lockr.get('Tropas_Ataque_Catapulta') : minha.catapultas;
            if (temPaladino) {
                _Data.knight = parseInt(Lockr.get('Tropas_Ataque_Paladino')) < parseInt(minha.paladino) ? Lockr.get('Tropas_Ataque_Paladino') : minha.paladino;
            }
            _Data.snob = parseInt(Lockr.get('Tropas_Ataque_Nobre')) < parseInt(minha.nobre) ? Lockr.get('Tropas_Ataque_Nobre') : minha.nobre;
  ...
  function terceira() {
    ...
    /* Script Escrito por ThiioM :) */
        for (let minha of minhas) {
            ...
            setTimeout(() => {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (i === minhas.length) {
                            setTimeout(function () {
                                 window.location.assign("https://brs1.tribalwars.com.br/game.php?village={game}&screen=snob&mode=coin&group=0"+pagina);
            ...
            }, aleatorio(230, 250) + (aleatorio(230, 250) * delay));
            delay++;

        }
  }

    if ($('.quickedit-label').length > 10) {
        ...
        if (!Lockr.get("Alvos") || Lockr.get("Alvos") === 'Coloque Aqui As Coordenadas E IDS De Ataque') {
                   alert('Não Tem Alvos');
        } else {
            if (!Lockr.get("Alvos_Nao_Atacados")) {
                resErro();
                selecionarAlvos();
            } else {
                if (Lockr.get("Alvos_Nao_Atacados").length > 0) {
                selecionarAlvos();
            } else {
                resErro();
                selecionarAlvos();
    ...
    setInterval(function () {
        ...
        if( $('h1').text() === "Solicitação bloqueada" ) {
            window.location.reload(true);
        }
